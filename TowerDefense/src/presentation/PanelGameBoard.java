...
public class PanelGameBoard extends JPanel implements MouseListener,
	...
	public void mouseClicked(MouseEvent e) {
		try {
			...
			if (sellMode) {
				...
				tower.targetLowestHealth();
			else if (highest)
				tower.targetHighestHealth();
			else if (closest)
				tower.targetClosest();
			else if (farthest)
				tower.targetFarthest();
			else {
				return;
			}
	...
	public void mouseMoved(MouseEvent e) {
		...
		Game game = view.getController().getGame();
		if (e.getX() / Map.CELL_PIXEL_SIZE < view.getModel().getGame()
				.getBoard().getMap().getWidth()
				&& e.getY() / Map.CELL_PIXEL_SIZE < view.getModel().getGame()
						.getBoard().getMap().getHeight()) {
			try {
				Tower tower = game.getBoard().getTower(
						new Point(e.getX() / Map.CELL_PIXEL_SIZE, e.getY()
								/ Map.CELL_PIXEL_SIZE));
				...
				if (upgradeMode && tower != null) {
					if (tower.getLevel() == 5) {
						if (tower.getIcon().getIconHeight() == 62) {
							this.setToolTipText("<html>"
									...
									+ " <b>(MAX)"
									+ "<br>Fire Rate: </b>"
									+ Math.round(100 * (tower.getFireRate()))
									/ value
									+ "<br><b>Range: </b>"
									+ tower.getRange()
									+ "<br><b>Slowing Power: </b>"
									+ (int) Math.round(100 * (tower
											.getSpecialmod())) / value
									+ "</html>");
						} else {
							this.setToolTipText("<html>"
									...
									+ tower.getRange()
									+ "<br><b>Damage: </b>"
									+ (int) Math.round(100 * (tower.getPower()))
									+ "</html>");
						}
					} else {
						if (tower.getIcon().getIconHeight() == 62) {
							this.setToolTipText("<html><b>Upgrade Cost: </b>"
									+ sc
									+ tower.getCost()
									+ "<br><b>Level: </b>"
									+ tower.getLevel()
									...
									/ value
									+ "--><b>"
									+ Math.round(100 * (tower.getFireRate() * 1.1))
									/ value
									+ "<br>Range: </b>"
									+ tower.getRange()
									+ "--><b>"
									...
									+ "--><b>"
									+ (int) Math.round(100 * (tower
											.getSpecialmod() - 0.05)) / value
									+ "</html>");
						}
						// needs to be adjusted if the upgrade attributes are
						// any different!!
						else {
							this.setToolTipText("<html><b>Upgrade Cost: </b>"
									+ sc
					...
					if (tower.getLevel() == 5) {
						this.setToolTipText("<html><b>Level: </b>"
								+ tower.getLevel()
								+ "<br><b>Selling Value: </b>" + sc
								+ tower.getValue()
								+ "<br><b>Targeting Mode: </b>"
								+ tower.getMode() + "</html>");
					} else {
						this.setToolTipText("<html><b>Level: </b>"
								+ tower.getLevel()
								+ "<br><b>Upgrade Cost: </b>" + sc
								+ tower.getCost()
								+ "<br><b>Selling Value: </b>" + sc
								+ tower.getValue()
								+ "<br><b>Targeting Mode: </b>"
	...
	public void paint(Graphics g) {
		...
		g.fillRect(0, 0, Map.CELL_PIXEL_SIZE * Map.MAX_WIDTH,
				Map.CELL_PIXEL_SIZE * Map.MAX_HEIGHT);
